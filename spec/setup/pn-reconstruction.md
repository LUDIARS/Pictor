# Pictor PN geometry demo

Windows x64, AVX2-capable CPU, Vulkan GPU/driver and Microsoft Visual C++ 2022
x64 runtime. Open `pictor_pn_demo.exe` in the supplied package.
...
cmake --build build --config Release --target pictor_pn_demo --parallel 4
```

Packaging a specific model with its textures is a local operation outside this
repository. Model files, textures, packaging scripts and capture catalogs for a
particular character are never committed. Distribution of source assets remains
subject to their existing terms.

## Captures through Excubitor

Capture services for a particular model are registered only in a local, untracked
Excubitor catalog. They run a staged `pictor_pn_demo.exe` with cwd `Pictor`.
