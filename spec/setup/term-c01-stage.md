# <term-c01> stage C production package

Build the existing Astra PN reconstruction demo with `pictor_<term-c01>_demo` in
Release, with tests disabled. Stage the executable and SPIR-V shaders in
`Pictor/build/<term-c01>-stage-demo/`. The generated stage services in the owner
...
keys extracted from KS `<term-c01>_turnR_ver1.fbx` / `jnt_Head`, normalized by the
...
The `pc-<term-c01>-stage-still` service writes an 800x960 BMP. The
`pc-<term-c01>-stage-video` service writes 432 BMPs at fixed simulated 24 fps.
...
```
node tools/encode_<term-c01>_stage.mjs <ffmpeg.exe> <stage-frames> <stage-c.mp4>
```
