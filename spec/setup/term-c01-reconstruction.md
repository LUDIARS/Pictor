# Pictor <term-c01> geometry demo

Windows x64, AVX2-capable CPU, Vulkan GPU/driver and Microsoft Visual C++ 2022
x64 runtime. Open `pictor_<term-c01>_demo.exe` in the supplied package.
...
cmake --build build --config Release --target pictor_<term-c01>_demo --parallel 4
```

Use `tools/package_<term-c01>_demo.ps1` with explicit `-Binary`, `-ShaderDirectory`,
...
The catalog has two opt-in entries: `pc-<term-c01>-demo` (interactive) and
`pc-<term-c01>-capture` (eight comparisons, then exit). Both run the staged
`Pictor/build/<term-c01>-demo/pictor_<term-c01>_demo.exe` with cwd `Pictor`.
