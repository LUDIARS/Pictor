# <term-c01>: research-based curved geometry in Pictor

The default executable now uses the direct polynomial raymarch path described in
[<term-c01>-raymarch.md](<term-c01>-raymarch.md). This document defines the shared PN
geometry and the optional sampled raster reference.

## SPEC-PC-<term-c01>-PN

`pictor_<term-c01>_demo.exe` imports the user's local `ch_<term-c01>_0000.fbx` as a
...
that target; `pictor_<term-c01>_demo` enables them and uses executable-relative assets.
...
- `demo/<term-c01>/pn_patch.*`: polynomial coefficients and evaluation.
- `demo/<term-c01>/pn_model.*`: imported adjacency, boundary policy, attributed sampling.
- `demo/<term-c01>/reconstruction.*`: comparison controls, camera, observation report.
...
- `tools/package_<term-c01>_demo.ps1`: explicit local asset/binary packaging.
