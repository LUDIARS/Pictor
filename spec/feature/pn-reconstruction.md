# Research-based curved geometry in Pictor

The default executable now uses the direct polynomial raymarch path described in
[pn-raymarch.md](pn-raymarch.md). This document defines the shared PN
geometry and the optional sampled raster reference.

## SPEC-PC-PN-RECONSTRUCTION

`pictor_pn_demo.exe` imports a user-supplied local skinned character FBX as a
...
that target; `pictor_pn_demo` enables them and uses executable-relative assets.
...
- `demo/pn_surface/pn_patch.*`: polynomial coefficients and evaluation.
- `demo/pn_surface/pn_model.*`: imported adjacency, boundary policy, attributed sampling.
- `demo/pn_surface/reconstruction.*`: comparison controls, camera, observation report.
