...
std::vector<GpuPnPatch> make_gpu_patches(const PnModel& model) {
    ...
    for (const auto& sub:source.submeshes) {
        const float kind=material_kind(sub.texture_basename);
...
float material_kind(const std::string& texture_basename) {
    if (texture_basename.find("Face")!=std::string::npos) return 1;
    if (texture_basename.find("Hair")!=std::string::npos) return 2;
    if (texture_basename.find("Crown")!=std::string::npos) return 3;
    return 4;
}
/// @implements SPEC-PC-POLYNOMIAL-MOTION
GpuPnPatch make_gpu_patch(const pictor::demo::PolynomialPatch& source,float kind) {
    ...
    out.lower[3]=source.constant_color?1.f:0.f;out.upper[3]=kind;
    return out;
}
}
