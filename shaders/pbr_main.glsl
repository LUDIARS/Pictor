// Pictor — PBR Fragment Shader (共有本体)
...
//
// エントリは薄いラッパから include される:
//   pbr.frag    — 従来どおり (GI 無し、 既存ホスト非破壊)
//   pbr_gi.frag — PICTOR_GI define 付き。 probe grid の間接光 (gi.glsl、
//                 set 2 binding 2/3) を ambient 項へ加算する opt-in 経路
//                 (spec/feature/gi-bake-realtime-design.md §2.5)。
// #version / #extension はラッパ側が宣言する。
