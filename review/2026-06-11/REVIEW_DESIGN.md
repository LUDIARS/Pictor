...
- **L-1**: 上位ライブラリ名 (PrivateGame / ergo_ui_kit / kuzu.profile.json) のコメント・文字列が前回から**増加** (`pictor_renderer.cpp:97`, `compiled_graph.h:76`, `tests/unit_visus_serializer_test.cpp:14` 等)。実依存ゼロは維持。「host」「上位エディタ」への言い換え推奨。
